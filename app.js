(function () {
  ...
  function createBlankWeightBalance() {
    return {
      ...
      momentRows: [
        { label: "Standard Empty Mass", weight: "", arm: "", moment: "" },
  ...
  function normalizeWeightBalancePayload(payload) {
    ...
    const sourceMomentRowsHaveLabels = sourceRows.some((row) => String(row && row.label ? row.label : "").trim() !== "");
    ...
    return {
      ...
      fuelRows: blank.fuelRows.map((row, index) => {
        const sourceRow = fuelRowsByLabel.get(row.label) || sourceFuelRows[index] || {};
        return {
          ...row,
          ...(sourceRow && typeof sourceRow === "object" ? sourceRow : {}),
          gallons: String(sourceRow && sourceRow.gallons != null ? sourceRow.gallons : ""),
          minutes: String(sourceRow && sourceRow.minutes != null ? sourceRow.minutes : ""),
          label: row.label,
        };
      }),
  ...
  function getWaypointSuggestionOptions() {
    ...
    (Array.isArray(state.catalog.waypoints) ? state.catalog.waypoints : []).forEach((record) => {
      const normalized = normalizeWaypointRecord(record);
      if (!normalized.name) return;
      const current = records.get(normalized.name) || { value: normalized.name, label: normalized.name, search: "" };
      current.search = Array.from(new Set(`${current.search} ${(normalized.aliases || []).join(" ")}`.trim().split(/\s+/).filter(Boolean))).join(" ");
      records.set(normalized.name, current);
    });
  ...
  function autofillDistanceBetweenWaypoints(distanceLegIndex) {
    ...
    targetLeg.distance = formatDistanceDisplayWithRounding(
      distanceNm,
  ...
  function render() {
    ...
    const viewChanged = navigationKey !== lastRenderedView;
    ...
    else if (state.view === "weight-balance") app.innerHTML = renderWeightBalanceScreen();
    ...
    else if (state.view === "weight-balance") wireWeightBalanceScreen();
    ...
    syncKioskGpsTrackingForView();
    if (!viewChanged) restoreViewScrollState();
    wireUtcAdminTrigger();
    wireFooterActions();
    wireBugReportModal();
  ...
  function getCurrentNavigationKey() {
    ...
    if (view === "admin") return `${view}:${String(state.admin.panel || "dashboard")}`;
    if (view === "additional-info") return `${view}:${String(state.meta.additionalInfoPanel || "")}`;
    return view;
  }
  ...
  function renderSetupScreen() {
    ...
    const showWeightBalanceResume = shouldShowWeightBalanceResumeButton();
    ...
    return `
      ...
      <main class="entry-page${isIpadDevice() ? " ipad-home-page" : ""}">
        ...
        <section class="setup-card wb-launch-card">
  ...
  function renderWeightBalanceScreen() {
    return `
      <div class="ui-scale">
      <main class="page wb-page">
  ...
  function convertNumericText(value, factor) {
    const numeric = num(value);
    if (numeric == null) return String(value ?? "");
    return formatWeightBalanceComputed(numeric * factor);
  }

  function getWeightConversionFactor(fromUnit, toUnit) {
    if (fromUnit === toUnit) return 1;
    if (fromUnit === "lbs" && toUnit === "kg") return 0.45359237;
    if (fromUnit === "kg" && toUnit === "lbs") return 1 / 0.45359237;
    return 1;
  }

  function getArmConversionFactor(fromUnit, toUnit) {
    if (fromUnit === toUnit) return 1;
    if (fromUnit === "in" && toUnit === "cm") return 2.54;
    if (fromUnit === "cm" && toUnit === "in") return 1 / 2.54;
    return 1;
  }

  function getFuelConversionFactor(fromUnit, toUnit) {
    if (fromUnit === toUnit) return 1;
    if (fromUnit === "gal" && toUnit === "l") return 3.785411784;
    if (fromUnit === "l" && toUnit === "gal") return 1 / 3.785411784;
    return 1;
  }

  function convertWeightBalanceWeightUnit(wb, toUnit) {
    const fromUnit = wb.units.weight === "kg" ? "kg" : "lbs";
    if (fromUnit === toUnit) return;
    const factor = getWeightConversionFactor(fromUnit, toUnit);
    wb.momentRows.forEach((row) => {
      row.weight = convertNumericText(row.weight, factor);
      row.moment = "";
    });
    wb.units.weight = toUnit;
    applyWeightBalanceTableMath(wb);
  }

  function convertWeightBalanceArmUnit(wb, toUnit) {
    const fromUnit = wb.units.arm === "cm" ? "cm" : "in";
    if (fromUnit === toUnit) return;
    const factor = getArmConversionFactor(fromUnit, toUnit);
    wb.momentRows.forEach((row) => {
      row.arm = convertNumericText(row.arm, factor);
      row.moment = "";
    });
    wb.units.arm = toUnit;
    applyWeightBalanceTableMath(wb);
  ...
  function convertWeightBalanceFuelUnit(wb, toUnit) {
    const fromUnit = wb.units.fuel === "l" ? "l" : "gal";
    if (fromUnit === toUnit) return;
  ...
  function getFuelDisplayValue(fuelRows, index, field) {
    ...
    if (field === "gallons") {
      if (row && row.label === "Fuel Required") return getFuelRequiredFuel(fuelRows);
      if (row && row.label === "Expected Fuel On Landing") return getExpectedFuelOnLanding(fuelRows);
  ...
  function isFuelCellReadOnly(index, field) {
    const rows = getWeightBalanceState().fuelRows;
  ...
  function applyMomentRowMath(row, changedField) {
    ...
    if ((changedField === "weight" || changedField === "arm") && num(row && row[changedField]) == null) {
      row.moment = "";
      return;
    }
    const weight = num(row && row.weight);
  ...
  function applyWeightBalanceTableMath(wb) {
    ...
    const takeOffTotals = wb.momentRows.slice(0, takeOffIndex).reduce((totals, row) => {
      const weight = num(row && row.weight);
      const moment = getMomentNumericValue(row);
      if (weight != null) {
        totals.weight += weight;
        hasTakeOffWeight = true;
      }
      if (moment != null) {
        totals.moment += moment;
        hasTakeOffMoment = true;
      }
      return totals;
    }, { weight: 0, moment: 0 });
    setComputedMomentRow(
      wb.momentRows[takeOffIndex],
      hasTakeOffWeight ? takeOffTotals.weight : null,
      hasTakeOffMoment ? takeOffTotals.moment : null
    );
    if (tripFuelIndex < 0 || landingIndex < 0) return;
    const tripFuelWeight = num(wb.momentRows[tripFuelIndex] && wb.momentRows[tripFuelIndex].weight);
    const tripFuelMoment = getMomentNumericValue(wb.momentRows[tripFuelIndex]);
    const landingWeight = hasTakeOffWeight && tripFuelWeight != null ? takeOffTotals.weight - tripFuelWeight : null;
    const landingMoment = hasTakeOffMoment && tripFuelMoment != null ? takeOffTotals.moment - tripFuelMoment : null;
    setComputedMomentRow(wb.momentRows[landingIndex], landingWeight, landingMoment);
  }
  ...
  function applyWeightBalancePreset(wb, presetName) {
    const preset = WEIGHT_BALANCE_PRESETS[presetName];
    if (!wb || !preset) return;
    wb.preset = presetName;
    wb.units.weight = "lbs";
    wb.units.arm = "in";
    wb.units.fuel = "gal";
    const rowsByLabel = new Map(wb.momentRows.map((row) => [row.label, row]));
    const rowValues = {
      "Standard Empty Mass": { weight: preset.basicWeight, arm: preset.basicArm },
      Fuel: { arm: preset.fuelArm },
      "Front Row": { arm: preset.frontRowArm },
    ...
    Object.entries(rowValues).forEach(([label, values]) => {
      ...
      if (!row) return;
      if (Object.prototype.hasOwnProperty.call(values, "weight")) row.weight = values.weight;
      if (Object.prototype.hasOwnProperty.call(values, "arm")) row.arm = values.arm;
      applyMomentRowMath(row, Object.prototype.hasOwnProperty.call(values, "weight") ? "weight" : "arm");
    });
    if (wb.fuelRows[0]) wb.fuelRows[0].gallons = preset.taxiFuel;
    applyWeightBalanceTableMath(wb);
    applyFuelTableMath(wb);
  }
  ...
  function clearWeightBalanceValues(wb) {
    ...
    const blank = createBlankWeightBalance();
    Object.assign(wb, blank, { unitsOpen });
  }
  ...
  function renderWeightBalancePanel() {
    ...
    return `
      <section class="setup-card wb-toolbar">
        <div class="wb-head">
          <h2>Weight and Balance</h2>
  ...
  function renderAdminWaypointAliasModal() {
    ...
    const selectedAliasIndex = Number(editor.selectedAliasIndex);
    const draft = String(editor.draft || "");
    return `
      <div class="activate-modal-layer admin-alias-overlay" id="admin-waypoint-alias-overlay">
        <section class="activate-modal-card admin-alias-panel" role="dialog" aria-modal="true" aria-label="Waypoint aliases">
          <header class="activate-modal-header">
            <div>
              <span class="activate-modal-kicker">Waypoint aliases</span>
              <h3>${escapeHtml(row.name || "Waypoint")}</h3>
            </div>
            <button class="action" id="admin-waypoint-alias-close" type="button" aria-label="Close aliases">Close</button>
          </header>
          <label class="activate-modal-field">
            <span>Alias</span>
            <input id="admin-waypoint-alias-draft" value="${escapeAttr(draft)}" placeholder="Type an alias" autocomplete="off" />
          </label>
          <div class="admin-alias-editor-actions">
            <button class="action primary" id="admin-waypoint-alias-submit" type="button">${selectedAliasIndex >= 0 ? "Update alias" : "Add alias"}</button>
            <button class="action" id="admin-waypoint-alias-delete" type="button" ${selectedAliasIndex >= 0 ? "" : "disabled"}>Delete</button>
          </div>
          <div class="admin-alias-list" aria-live="polite">
            ${aliases.length ? aliases.map((alias, aliasIndex) => `
              <button class="admin-alias-item${selectedAliasIndex === aliasIndex ? " active" : ""}" data-admin-waypoint-alias-select="${rowIndex}:${aliasIndex}" type="button">
                ${escapeHtml(alias)}
              </button>
            `).join("") : '<p class="admin-alias-empty">No aliases added yet.</p>'}
          </div>
        </section>
      </div>
    `;
  ...
  function wireSetup() {
    ...
    const openWeightBalanceButton = document.getElementById("open-weight-balance");
    if (openWeightBalanceButton) {
      openWeightBalanceButton.addEventListener("click", () => {
        if (hasMeaningfulWeightBalanceData() && !window.confirm("Are you sure you want to lose current weight and balance progress and make a new sheet?")) return;
        state.navlog.weightBalance = createBlankWeightBalance();
        state.meta.hasOpenedWeightBalance = true;
        state.meta.lastNonDocView = "setup";
        state.view = "weight-balance";
        render();
      });
    }
    const resumeWeightBalanceButton = document.getElementById("resume-weight-balance");
    if (resumeWeightBalanceButton) {
      resumeWeightBalanceButton.addEventListener("click", () => {
        state.meta.hasOpenedWeightBalance = true;
        state.meta.lastNonDocView = "setup";
        state.view = "weight-balance";
        render();
  ...
  function wireWeightBalanceScreen() {
    const backButton = document.getElementById("back-from-weight-balance");
    if (backButton) {
  ...
  function wireWeightBalancePanel() {
    ...
    if (weightUnitSelect) {
      weightUnitSelect.addEventListener("change", (event) => {
        const wb = getWeightBalanceState();
        state.meta.hasOpenedWeightBalance = true;
        convertWeightBalanceWeightUnit(wb, event.target.value === "kg" ? "kg" : "lbs");
        render();
      });
    }
    ...
    if (armUnitSelect) {
      armUnitSelect.addEventListener("change", (event) => {
        const wb = getWeightBalanceState();
        state.meta.hasOpenedWeightBalance = true;
        convertWeightBalanceArmUnit(wb, event.target.value === "cm" ? "cm" : "in");
        render();
      });
    }
    ...
    if (fuelUnitSelect) {
      fuelUnitSelect.addEventListener("change", (event) => {
        const wb = getWeightBalanceState();
        state.meta.hasOpenedWeightBalance = true;
        convertWeightBalanceFuelUnit(wb, event.target.value === "l" ? "l" : "gal");
        render();
      });
    }
  ...
  function wireAdminPanel() {
    ...
    const aliasDraftInput = document.getElementById("admin-waypoint-alias-draft");
    if (aliasDraftInput) {
      aliasDraftInput.addEventListener("input", () => {
        state.admin.waypointAliasEditor.draft = String(aliasDraftInput.value || "");
      });
      aliasDraftInput.addEventListener("keydown", (event) => {
        if (event.key !== "Enter") return;
        event.preventDefault();
        document.getElementById("admin-waypoint-alias-submit")?.click();
      });
    }
    document.querySelectorAll("[data-admin-waypoint-alias-select]").forEach((button) => {
      button.addEventListener("click", () => {
        const [rowIndexText, aliasIndexText] = String(button.getAttribute("data-admin-waypoint-alias-select") || "").split(":");
    ...
    if (aliasDeleteButton) {
      aliasDeleteButton.addEventListener("click", async () => {
        ...
        const rows = normalizeWaypointRows(state.admin.waypointForm.rows);
        if (!rows[rowIndex] || aliasIndex < 0 || aliasIndex >= rows[rowIndex].aliases.length) return;
  ...
  async function signOutAdmin() {
    ...
    state.admin.waypointForm = createEmptyWaypointForm();
    state.admin.waypointAliasEditor = { open: false, rowIndex: 0, selectedAliasIndex: -1, draft: "" };
    state.admin.rpcRegistryForm = createEmptyRpcRegistryForm();
    state.admin.chartForm = createEmptyChartForm();
  ...
  function shouldShowWeightBalanceResumeButton() {
    return state.meta.hasOpenedWeightBalance && hasMeaningfulWeightBalanceData();
  }
  ...
  function hasMeaningfulWeightBalanceData() {
